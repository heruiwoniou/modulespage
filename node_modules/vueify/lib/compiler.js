...
compiler.compile = function (content, filePath, cb) {
  ...
  function mergeParts (parts) {
    ...
    //replace script
    output = output.replace(/exports\.default\s*=/g , 'var component =' );
    // template
    var template = extract(parts, 'template');
    var moduleName = function(){
        return filePath.split('\\').pop().split('').reverse().slice(4).reverse().join('');
    }()
    if (template) {
      output +=
    ...
    if(output)
    {
      var variableREG=/([\n\t]*(var\s+([^\s]+)\s+=\s+)(require\((["'][^\s]*?['"])\);))/ig ,
      quoteREG = /([\n\t]*(require\((["'][^\s]*?['"])\);))/ig ,
      amdstring = '', args = '',match;
      output = '\t' + output.replace(/(\n)([^\n])/g,'$1\t$2').replace(/\tObject\.defineProperty[\s\S]*?\}\);/,'');
      while(match = variableREG.exec(output))
      {
        amdstring += ' , ' + match[5];
        args += ' , ' + match[3]
      };
      output = output.replace(variableREG,'');
      while(match = quoteREG.exec(output))
      {
        amdstring += ' , ' + match[3];
      };
      output = output.replace(quoteREG,'');
      output =
      '(function(factory){' + '\n' +
        '\t' + 'if (typeof define === "function" && define.amd) {' + '\n' +
        '\t\t' + 'define( ["vue"'+ amdstring +'] , factory)' + '\n' +
